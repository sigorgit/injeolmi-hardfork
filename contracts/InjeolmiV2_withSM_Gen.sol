...
//not finished
contract InjeolmiV2SMGen is Ownable, IInjeolmiV2SMnoGen {
    ...
    uint256 private initial_rTotal = (uint256(-1).div(TOTAL_SUPPLY)).mul(TOTAL_SUPPLY);

    mapping(uint256 => uint256) private _rTotal; //_rTotal[gen].
    mapping(uint256 => uint256) private _revisionRate; //_revisionRate[gen].
    ...
    function _increaseGen() internal {
        uint256 lastRate = _getRate();
        generation++;
        _rTotal[generation] = initial_rTotal;
        uint256 newRate = _getRate();
        _revisionRate[generation] = newRate.div(lastRate);
        // emit IncreaseGeneration(generation); TODO
    }
    ...
    function addToNoDistribution(address account) public onlyOwner {
        ...
        if (rOwned > 0) {
            _aInfo._tOwned = tokenFromReflection(rOwned, generation);
        }
    ...
    function removeFromNoDistribution(address account) public onlyOwner {
        ...
        _rTotal[generation] = _rTotal[generation].sub(diff);
    ...
    constructor() public {
        _rTotal[0] = initial_rTotal;
        _userInfo[msg.sender]._rOwned = initial_rTotal;
    ...
    function balanceOf(address user) public view returns (uint256) {
        ...
        return tokenFromReflection(_uInfo._rOwned, _uInfo.userGen);
    }

    function tokenFromReflection(uint256 rAmount, uint256 _generation) internal view returns (uint256) {
        uint256 currentRate = _getRate();
        require(_generation <= generation);
        for(uint256 i = _generation; i < generation; i++) {
            rAmount = rAmount.mul(_revisionRate[i]);    //TODO overflow
        }
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        uint256 rSupply = _rTotal[generation];
        uint256 tSupply = TOTAL_SUPPLY;

        for (uint256 i = 0; i < noDistribution.length; i++) {
            UserInfo memory _uInfo = _userInfo[noDistribution[i]];

            if (_uInfo._rOwned > rSupply || _uInfo._tOwned > tSupply) return (_rTotal[generation], TOTAL_SUPPLY);
            rSupply = rSupply.sub(_uInfo._rOwned);
            tSupply = tSupply.sub(_uInfo._tOwned);
        }

        if (rSupply < _rTotal[generation].div(TOTAL_SUPPLY)) {
            console.log("here!", rSupply, _rTotal[generation].div(TOTAL_SUPPLY), tSupply);
            return (_rTotal[generation], TOTAL_SUPPLY);
        }
    ...
    function _transfer(
        ...
        //noFeeDistribution
        if (isExcludedFrom(from) || isExcludedTo(to)) {
    ...
    function _reflectFee(uint256 rFee, uint256 tFee) internal {
        _rTotal[generation] = _rTotal[generation].sub(rFee);
